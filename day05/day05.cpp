...
auto process(std::vector<int> program, int input) {
    size_t ip{ 0 };
    while (static_cast<opcode>(program.at(ip)) != opcode::halt) {
        int modes{ program.at(ip) / 100 };
        mode param1_mode = modes % 10 == 0 ? mode::position : mode::immediate;
        modes /= 10;
        mode param2_mode = modes % 10 == 0 ? mode::position : mode::immediate;
        modes /= 10;
        mode param3_mode = modes % 10 == 0 ? mode::position : mode::immediate;

        opcode op{ program.at(ip) < 100 ? static_cast<opcode>(program.at(ip))
            : static_cast<opcode>(program.at(ip) % 100) };
...
int main() {
    ...
    int part_one_answer{ process(program, 1) };
    std::cout << "part_one_answer: " << part_one_answer << "\n";
    assert(part_one_answer == 7265618);

    int part_two_answer{ process(program, 5) };
    std::cout << "part_two_answer: " << part_two_answer << "\n";
    assert(part_two_answer == 7731427);
